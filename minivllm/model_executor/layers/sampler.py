...
def _apply_penalties(
    ...
    logits[indices] -= presence_penalties.unsqueeze(1) * presence_mask
    logits[indices] -= frequency_penalties.unsqueeze(1) * bin_counts
    return logits
...
def _get_penalties(
    ...
    for i, seq_group in enumerate(input_metadata.seq_groups):
        ...
        if i < input_metadata.num_prompts:
            ...
            # A generation token.
            # have totally len(seq_ids) tokens for this group.
...
def _get_output_tokens(
    ...
    for i, seq_group in enumerate(input_metadata.seq_groups):
        seq_ids, _ = seq_group
        if i < input_metadata.num_prompt:
            # A prompt input.
...
def _sample_from_prompt(
    ...
    if sampling_params.use_beam_search:
        ...
        next_token_ids = torch.argmax(prob)
        next_token_ids = [next_token_ids.item()]
    else:
        # Random sampling.
...
def _sample_from_generation_tokens(
    seq_ids: List[int],
    probs: torch.Tensor,        # 用于 topp、topk 搜索
    logprobs: torch.Tensor,     # 用于全局 beam search
    seq_logprobs: List[float],
    sampling_params: SamplingParams,
) -> Tuple[List[int], List[int]]:
    # NOTE (by original author): sampling_params.best_of can be greater than
    # len(seq_ids) because some sequences in the group might have
    # been already terminated.
    if sampling_params.use_beam_search:
        # Beam search
        seq_logprobs = torch.tensor(
            seq_logprobs, dtype=logprobs.dtype, device=logprobs.device)
        logprobs = logprobs + seq_logprobs.unsqueeze(dim=1)

        vocab_size = logprobs.size(-1)
        beam_width = len(seq_ids)
        # 找所有可能中最大的，有些分支可能会汇聚到同一个序列
        ...
        #
        for parent_seq_id, token_id in zip(beam_seq_ids, token_ids):
            if parent_seq_id not in beam_outputs:
                beam_outputs[parent_seq_id] = (parent_seq_id, token_id)
            else:
                outstanding_beams.append((parent_seq_id, token_id))
        ...
        next_token_ids = [int(next_token_ids.item())]
...
def _sample(
    ...
    for i, seq_group in enumerate(input_metadata.seq_groups):
        ...
        if i < input_metadata.num_prompts:
            ...
            # Generate the next token from generation tokens.
