...
class SamplingParams:
    ...
    Args:
        ...
        logprobs: 每个输出token返回的logprob数量
    """
    def __init__(
        self,
        n: int = 1,
    ...
    def _verify_args(self) -> None:
        ...
        assert self.presence_penalty >= -2.0 and self.presence_penalty <= 2.0, \
            f"presence_penalty must be in [-2, 2], got {self.presence_penalty}."
        assert self.frequency_penalty >= -2.0 and self.frequency_penalty <= 2.0, \
