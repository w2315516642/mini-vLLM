...
exec env CUDA_VISIBLE_DEVICES="${DECODE_CUDA_DEVICES:-1}" python \
  ...
  --pd-endpoint-id "${DECODE_ENDPOINT_ID:-d}" \
  --pd-hostname "${DECODE_DATA_ADDRESS:-127.0.0.1:14100}" \
  --pd-peer-endpoint-id "${PREFILL_ENDPOINT_ID:-p}" \
  --pd-peer-hostname "${PREFILL_DATA_ADDRESS:-127.0.0.1:14000}" \
  --control-address "${DECODE_CONTROL_ADDRESS:-127.0.0.1:15100}" \
