...
DeltaNet convolution/recurrent state. With DSpark enabled, its Draft K/V uses
the same physical block mapping and is transferred in the same data-plane
batch. P samples the first output token and transfers state after the full
prompt, so D starts by consuming that output token. MTP proposals and M-RoPE
positions are control metadata; image/video feature tensors remain on P.

DSpark proposals are owned by the role that will execute the next Target
verification. P materializes prompt-side Draft K/V but deliberately does not
attach a proposal to the handoff. D consumes P's first token, appends its
target feature to Draft K/V, and then starts proposing. This avoids sending a
full `draft_width * vocab_size` stochastic q distribution through the control
plane.
...
For DSpark, add the same options to both role commands:

```bash
--draft-model RadixArk/Qwen3.8-27B-DSpark \
--num-speculative-tokens 7 --speculative-adaptive
```
...
- The published Qwen3.8 DSpark path is text-only; multimodal requests continue
  to use Qwen3.8 without DSpark proposals.
